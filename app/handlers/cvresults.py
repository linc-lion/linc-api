...
class CVResultsHandler(BaseHandler):
    ...
    def get(self, res_id=None, xlist=None):
        if res_id:
            if res_id == 'list':
                objs = yield self.db.cvresults.find().to_list(None)
                self.set_status(200)
                self.finish(self.json_encode({'status': 'success', 'data': self.list(objs)}))
            else:
                query = self.query_id(res_id)
                objs = yield self.db.cvresults.find_one(query)
                if objs:
                    if not xlist:
                        objres = dict(objs)
                        ...
                        animl = yield self.db[self.animals].find().to_list(None)
                        ...
                        for i in mp:
                            ...
                            # get the animal
                            aobj = yield self.db[self.animals].find_one({'iid': objres['id']})
                            if aobj:
                                ...
                                if img:
                                    ...
                                    if len(img) > 0:
                                        ...
                                        objres['thumbnail'] = ''
                                        objres['image'] = ''
                                imgss = yield self.db.imagesets.find_one({'iid': aobj['primary_image_set_iid']})
                                if imgss:
                        ...
                        cvreq = yield self.db.cvrequests.find_one({'iid': objs['cvrequest_iid']})
                        assoc = {'id': None, 'name': None}
                        reqstatus = '-'
                        if cvreq:
                            reqid = cvreq['iid']
                            reqstatus = cvreq['status']
                            imgset = yield self.db.imagesets.find_one({'iid': cvreq['image_set_iid']})
                            if imgset:
                                assoc['id'] = imgset['animal_iid']
                                if imgset['animal_iid']:
                                    lname = yield self.db[self.animals].find_one({'iid': imgset['animal_iid']})
                                    if lname:
    ...
    def delete(self, res_id=None):
        ...
        if res_id:
            query = self.query_id(res_id)
            updobj = yield self.db.cvresults.find_one(query)
            if updobj:
